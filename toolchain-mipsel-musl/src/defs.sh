...
LINUX_HEADERS_URL=https://bitbucket.org/padavan/rt-n56u/toolchain-mipsel/src/mipsel-linux-uclibc-toolchain/kernel-headers-3.4.x.tar.bz2
...
[ -n "$CC_BASE_PREFIX" -a -z "$CC_PREFIX" ] && CC_PREFIX="$CC_BASE_PREFIX/$TARGET_TOOLCHAIN"
...
patch_source() {
    ...
    if [ ! -e patched ]
    then
        if [ -d "$MUSL_CC_BASE/patches/$BD" ]
        then
            for i in "$MUSL_CC_BASE/patches/$BD"/*.patch  ; do
                if [ -f "$i" ] ; then patch -p1 < "$i" || die "Failed to apply patch $i to $BD" ; fi
            done
            touch patched_within_dir
        else
