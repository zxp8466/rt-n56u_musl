...
if [ ! -e "$CC_PREFIX"/usr ]
then
    mkdir -p "$CC_PREFIX"/usr
fi
if [ "$WITH_SYSROOT" = "yes" ]
then
    SYSROOT_FLAGS="--with-sysroot=""$CC_PREFIX"
fi
...
buildinstall 1 gcc-$GCC_VERSION --target=$TRIPLE $SYSROOT_FLAGS \
    --enable-languages=c --with-newlib --disable-libssp --disable-nls --without-isl --without-cloog \
...
then
    ...
    cp -rf "$MUSL_CC_BASE/include" "$CC_PREFIX" 
    ln -sf ../include "$CC_PREFIX"/usr/include
...
#    PREFIX="$CC_PREFIX/$TRIPLE"
    ...
    buildinstall 2 gcc-$GCC_VERSION --target=$TRIPLE $SYSROOT_FLAGS \
        ...
        --with-headers="$CC_PREFIX/include" \
        --with-slibdir="$CC_PREFIX/lib" \
        $GCC_MULTILIB_CONFFLAGS \
        $GCC_CONFFLAGS
fi
